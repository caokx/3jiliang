/********************************************************************
Filename:       Dl698_Crc.h
Revised:        $Date: 2017-05-22 $
Revision:       $Revision: V0.1 $
Author:

Description:   DL698 crc file.
**********************************************************************/
...
/*
 * CRC校验函数，使用时分两步：先将要校验的内容进行校验获取校验码；将刚获取的校验码与帧里面保存的校验码对比。
 * 如果二者相等则校验通过，否则校验错误。
 */
int fcs16(unsigned char* buff, int length)
{
    unsigned short fcs = 0xFFFF;
    //int i;
    while (length--)
    {
        fcs = (int)(((fcs >> 8) ^ fcstab[(fcs ^ *buff++) & 0xFF]) & 0xFFFF);
    }
