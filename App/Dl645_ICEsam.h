...
#define MAX_ICEsamCom_SIZE 180

typedef struct
{
    u16     EFlag;          //通讯状态
    u16     Len;
    u16     Ic_Comm_RxPtr;  // 接受数据指针长度
...
typedef struct
{
    u8  CardPricePara1Buf[10]; // 第1套费率文件
    u8  CardPricePara2Buf[10]; // 第2套费率文件
    u8  CardParaBuf[2];     // 报警金额1，报警金额2，电压、电流变比
    u8  CardPriceSwitchTime[3]; // 两套分时费率切换时间
    u8  CardMoneyBag[2];        // 钱包文件 0-7 钱包金额  8-15 购电次数
    u8  CardUserNo[3];          // 客户编号
    u8  CardUpdataFlag[2];      // 参数更新标志位
